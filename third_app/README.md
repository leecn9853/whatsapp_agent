...
报表技能（`cost-report`、`alipay-report`）使用的 **Mock 数据服务**，模拟第三方业务 HTTP 接口。Demo 阶段专用；上线后应替换为真实业务 API。

启动：仓库根目录 `make third-app`，或 `uv run python main.py`（监听 **8800**）。

## 接口

`GET /api/electronics/orders` — 外贸电子产品行业订单数据，不分页，一次性返回约 150 条
...
访问 http://127.0.0.1:8800/docs 可查看 Swagger 文档。
