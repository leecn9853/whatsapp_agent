...
每次访问最新值，不能在模块顶层 `from src.agent_server._runtime import agent`
后直接用这个名字（那样拿到的是构建之前的 None，不会跟着 lifespan 更新）。

pool 是给 routes/threads.py 里"列出所有 thread"这条对 checkpoints 表的原始 SQL
...
# 且删除线程要等正在跑的 run 结束再删；routes/runs.py 和 routes/threads.py 都要用
# 同一份锁登记表，所以放在这个共享模块里，不是各自维护一份。
