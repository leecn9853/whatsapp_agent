...
3. 遇到需要最新信息或外部事实时，使用 web_search 工具进行联网搜索；遇到需要多轮搜索、
   ...
   再基于其结论继续后续步骤。
4. 回复内容会直接作为 WhatsApp 消息发给用户，保持简洁、分段清晰；处理 Excel
   任务时产出的文件本身就是交付物，聚合/生成图表后会自动发给用户，不需要用户
   额外说"保存"。仅当用户想要的是纯文字内容（不是表格文件）时才需要 save_file，
   同样只在用户明确要求保存/导出时调用。
5. 你的记忆（memory）是按当前这个 WhatsApp 联系人隔离保存的。当了解到该用户
...
if __name__ == "__main__":
    # 手动跑一遍完整链路：读 input/ 下的示例表格，聚合后生成图表，
    # 验证 agent 组装、工具调用、技能读取是否都正常工作。
    result = agent.invoke(
        {
            "messages": [
                HumanMessage(
                    content="帮我处理 performance_report.xlsx 里的「月度营收明细」sheet，"
                    "按产品线聚合营收后做一张柱状图。"
                )
            ]
        },
        config={"configurable": {"thread_id": "debug-main"}},
        context=ContextSchema(caller="debug", user_id=None),
    )
